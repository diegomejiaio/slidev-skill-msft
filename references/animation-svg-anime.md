...
Two representative components that exercise every pattern above:

- `story-sequence.vue` — multi-step sequence diagram with HTML bubbles via
  `<foreignObject>`, double-wrapped numbered badges, a pulse ring on the active
  actor, and per-step `animate()` for clean reverse navigation.
- `architecture-tiers-animated.vue` — 3-tier architecture hero (frontend /
  middle tier / backend services) with stroke-drawn connectors, staggered card
  reveals, floating chip labels with progressive fade-in, and looping data
  packets.
